...
const YearlyDataSchema = new Schema({
  ...
  unit: {
    type: String,
    trim: true
  }, // e.g., "tons", "thousand ML", "million ML", "bags"
  source: {
    type: String,
    trim: true,
    required: true
...
const WasteMetricSchema = new Schema({
  category: {
    type: String,
    required: true,
    trim: true,
  ...
  subcategory: {
    type: String,
    trim: true
  }, // e.g., "recyclable", "boiler_ash", "general", "effluent_discharged", "water_treatment", "liners", "tonne_bags", "packaging_material"
  metric_name: {
    type: String,
    required: true,
    trim: true
  ...
  // For single value (e.g., total number of initiatives)
  single_value: {
  ...
  // For list data (e.g., list of waste management measures)
  list_data: [{
    item: String,
    count: Number,
    details: String,
...
// Main Waste Management Data Model
const WasteManagementDataSchema = new Schema({
  company: {
    type: Schema.Types.ObjectId,
    ref: "Company",
    required: true,
  ...
  // Metrics data
  metrics: [WasteMetricSchema],

  // Summary statistics (auto‑calculated or manually set)
  summary_stats: {
    total_recyclable_waste: { type: Number, default: 0 },
    total_boiler_ash: { type: Number, default: 0 },
    total_general_waste: { type: Number, default: 0 },
    total_effluent_discharged: { type: Number, default: 0 },
    average_water_treatment: { type: Number, default: 0 },
    total_liners_used: { type: Number, default: 0 },
    total_tonne_bags: { type: Number, default: 0 },
    total_packaging_material: { type: Number, default: 0 },
    last_updated: Date
  },
  ...
  previous_version: { type: Schema.Types.ObjectId, ref: "WasteManagementData" },
  restored_from: { type: Schema.Types.ObjectId, ref: "WasteManagementData" },
...
// Indexes
WasteManagementDataSchema.index({ company: 1, is_active: -1 });
WasteManagementDataSchema.index({ company: 1, "data_period_end": -1 });
WasteManagementDataSchema.index({ company: 1, "metrics.category": 1 });
WasteManagementDataSchema.index({ company: 1, "metrics.metric_name": 1 });

module.exports = mongoose.model("WasteManagementData", WasteManagementDataSchema);