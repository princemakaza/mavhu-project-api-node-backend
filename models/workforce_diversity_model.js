...
const WorkforceMetricSchema = new Schema({
  category: {
    type: String,
    required: true,
    trim: true,
  ...
  subcategory: {
    type: String,
    trim: true,
  }, // e.g., "female_employees", "male_employees", "female_recruits", "male_recruits", "under_30", "30_to_50", "over_50", "18_to_35", "36_to_50", "51_to_54", "55_to_65"
  metric_name: {
    type: String,
    required: true,
    trim: true,
  ...
  // For single value (e.g., percentage representation)
  single_value: {
  ...
  // For list data (e.g., list of diversity initiatives)
  list_data: [
    {
      item: String,
      count: Number,
...
// Main Workforce & Diversity Data Model
const WorkforceDiversityDataSchema = new Schema({
  company: {
    type: Schema.Types.ObjectId,
    ref: "Company",
    required: true,
  ...
  // Metrics data
  metrics: [WorkforceMetricSchema],

  // Summary statistics (auto‑calculated or manually set)
  summary_stats: {
    total_female_employees: { type: Number, default: 0 },
    total_male_employees: { type: Number, default: 0 },
    total_female_recruits: { type: Number, default: 0 },
    total_male_recruits: { type: Number, default: 0 },
    average_female_representation: { type: Number, default: 0 },
    average_turnover_rate: { type: Number, default: 0 },
    last_updated: Date,
  },
  ...
  previous_version: {
    type: Schema.Types.ObjectId,
    ref: "WorkforceDiversityData",
  },
  restored_from: { type: Schema.Types.ObjectId, ref: "WorkforceDiversityData" },
...
// Indexes
WorkforceDiversityDataSchema.index({ company: 1, is_active: -1 });
WorkforceDiversityDataSchema.index({ company: 1, data_period_end: -1 });
WorkforceDiversityDataSchema.index({ company: 1, "metrics.category": 1 });
WorkforceDiversityDataSchema.index({ company: 1, "metrics.metric_name": 1 });

module.exports = mongoose.model(
  "WorkforceDiversityData",
  WorkforceDiversityDataSchema,
);
