// File: controllers/biodiversity_landuse_controller.js
const biodiversityLandUseService = require("../services/biodiversity_data_service");
...
class BiodiversityLandUseController {
  /**
  ...
  importFile = asyncHandler(async (req, res) => {
    ...
    const record = await biodiversityLandUseService.importFromFile(
      req.file.buffer,
  ...
  importJSON = asyncHandler(async (req, res) => {
    ...
    const record = await biodiversityLandUseService.importFromJSON(
      req.body.data,
      companyId,
      userId,
      metadata,
  ...
  /**
   * Get all biodiversity records for a company
   */
  getCompanyRecords = asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const { include_inactive } = req.query;

    const records = await biodiversityLandUseService.getCompanyRecords(
      companyId,
  ...
  getRecordById = asyncHandler(async (req, res) => {
    ...
    const record = await biodiversityLandUseService.getRecordById(
      recordId,
      companyId,
    );
  ...
  getMetricsByCategory = asyncHandler(async (req, res) => {
    ...
    let metrics = await biodiversityLandUseService.getMetricsByCategory(
      companyId,
      category,
    );
  ...
  getTimeSeriesData = asyncHandler(async (req, res) => {
    ...
    const timeSeriesData = await biodiversityLandUseService.getTimeSeriesData(
      companyId,
      metricName,
      category,
    );
  ...
  upsertMetric = asyncHandler(async (req, res) => {
    ...
    const record = await biodiversityLandUseService.upsertMetric(
      companyId,
      metricData,
      userId,
    );
  ...
  deleteMetric = asyncHandler(async (req, res) => {
    ...
    const record = await biodiversityLandUseService.deleteMetric(
      companyId,
      metricId,
      userId,
    );
  ...
  getSummaryStats = asyncHandler(async (req, res) => {
    ...
    const summary = await biodiversityLandUseService.getSummaryStats(companyId);

    if (!summary) {
      throw new AppError(
        "No biodiversity data found for this company",
        404,
        "NO_DATA_FOUND",
      );
    }
  ...
  validateData = asyncHandler(async (req, res) => {
    ...
    const validationResult =
      await biodiversityLandUseService.validateData(companyId);
  ...
  updateVerificationStatus = asyncHandler(async (req, res) => {
    ...
    const record = await biodiversityLandUseService.updateVerificationStatus(
      companyId,
      status,
      userId,
      notes,
  ...
  getDataVersions = asyncHandler(async (req, res) => {
    ...
    const versions =
      await biodiversityLandUseService.getDataVersions(companyId);
  ...
  restoreVersion = asyncHandler(async (req, res) => {
    ...
    const restoredRecord = await biodiversityLandUseService.restoreVersion(
      companyId,
      versionId,
      userId,
    );
  ...
  /**
   * Create new biodiversity record manually
   */
  createRecord = asyncHandler(async (req, res) => {
    requireFields(req.body, ["metrics", "version"]);
    ...
    const record = await biodiversityLandUseService.createRecord(
      recordData,
      userId,
    );
  ...
  /**
   * Export biodiversity data as CSV
   */
  exportCSV = asyncHandler(async (req, res) => {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=biodiversity-data-${companyId}-${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  getMetricsByDataType = asyncHandler(async (req, res) => {
    ...
    const metrics = record.metrics.filter(
      (metric) => metric.data_type === dataType && metric.is_active,
    );
...
module.exports = new BiodiversityLandUseController();
