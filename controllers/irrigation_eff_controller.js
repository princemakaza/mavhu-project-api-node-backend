const irrigationService = require("../services/irrigation_eff_service");
...
class IrrigationEfficiencyController {
  /**
  ...
  importFile = asyncHandler(async (req, res) => {
    ...
    const record = await irrigationService.importFromFile(
      req.file.buffer,
  ...
  importJSON = asyncHandler(async (req, res) => {
    ...
    const record = await irrigationService.importFromJSON(
      req.body.data,
      companyId,
      userId,
      metadata
  ...
  /**
   * Get all irrigation efficiency records for a company
   */
  getCompanyRecords = asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const { include_inactive } = req.query;

    const records = await irrigationService.getCompanyRecords(
      companyId,
  ...
  getRecordById = asyncHandler(async (req, res) => {
    ...
    const record = await irrigationService.getRecordById(recordId, companyId);
  ...
  getMetricsByCategory = asyncHandler(async (req, res) => {
    ...
    let metrics = await irrigationService.getMetricsByCategory(
      companyId,
      category
    );
  ...
  getTimeSeriesData = asyncHandler(async (req, res) => {
    ...
    const timeSeriesData = await irrigationService.getTimeSeriesData(
      companyId,
      metricName,
      category
    );
  ...
  upsertMetric = asyncHandler(async (req, res) => {
    ...
    const record = await irrigationService.upsertMetric(
      companyId,
      metricData,
      userId
    );
  ...
  deleteMetric = asyncHandler(async (req, res) => {
    ...
    const record = await irrigationService.deleteMetric(
      companyId,
      metricId,
      userId
    );
  ...
  getSummaryStats = asyncHandler(async (req, res) => {
    ...
    const summary = await irrigationService.getSummaryStats(companyId);

    if (!summary) {
      throw new AppError(
        "No irrigation efficiency data found for this company",
        404,
        "NO_DATA_FOUND"
      );
    }
  ...
  validateData = asyncHandler(async (req, res) => {
    ...
    const validationResult = await irrigationService.validateData(companyId);
  ...
  updateVerificationStatus = asyncHandler(async (req, res) => {
    ...
    const record = await irrigationService.updateVerificationStatus(
      companyId,
      status,
      userId,
      notes
  ...
  getDataVersions = asyncHandler(async (req, res) => {
    ...
    const versions = await irrigationService.getDataVersions(companyId);
  ...
  restoreVersion = asyncHandler(async (req, res) => {
    ...
    const restoredRecord = await irrigationService.restoreVersion(
      companyId,
      versionId,
      userId
    );
  ...
  /**
   * Create new irrigation efficiency record manually
   */
  createRecord = asyncHandler(async (req, res) => {
    ...
    const record = await irrigationService.createRecord(recordData, userId);

    res.status(201).json({
      success: true,
      message: "Irrigation efficiency record created successfully",
      data: record,
    });
  });

  /**
   * Export irrigation efficiency data as CSV
   */
  exportCSV = asyncHandler(async (req, res) => {
    const { companyId } = req.params;

    const record = await irrigationService.getRecordById(null, companyId);
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=irrigation-efficiency-${companyId}-${
...
module.exports = new IrrigationEfficiencyController();