const esgService = require("../services/overall_esg_service");
...
class OverallESGController {
  ...
  importFile = asyncHandler(async (req, res) => {
    ...
    const record = await esgService.importFromFile(
      req.file.buffer,
  ...
  importJSON = asyncHandler(async (req, res) => {
    ...
    const record = await esgService.importFromJSON(
      req.body.data,
      companyId,
      userId,
      metadata,
  ...
  /**
   * Get all overall ESG records for a company
   */
  getCompanyRecords = asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const { include_inactive } = req.query;

    const records = await esgService.getCompanyRecords(
      companyId,
  ...
  getRecordById = asyncHandler(async (req, res) => {
    ...
    const record = await esgService.getRecordById(recordId, companyId);
  ...
  /**
   * Get metrics by category (e.g., environmental, social, governance)
   */
  getMetricsByCategory = asyncHandler(async (req, res) => {
    const { companyId, category } = req.params;
    const { subcategory } = req.query;

    let metrics = await esgService.getMetricsByCategory(companyId, category);
  ...
  getTimeSeriesData = asyncHandler(async (req, res) => {
    ...
    const timeSeriesData = await esgService.getTimeSeriesData(
      companyId,
      metricName,
      category,
    );
  ...
  upsertMetric = asyncHandler(async (req, res) => {
    ...
    const record = await esgService.upsertMetric(companyId, metricData, userId);
  ...
  deleteMetric = asyncHandler(async (req, res) => {
    ...
    const record = await esgService.deleteMetric(companyId, metricId, userId);
  ...
  getSummaryStats = asyncHandler(async (req, res) => {
    ...
    const summary = await esgService.getSummaryStats(companyId);

    if (!summary) {
      throw new AppError(
        "No overall ESG data found for this company",
        404,
        "NO_DATA_FOUND",
      );
    }
  ...
  validateData = asyncHandler(async (req, res) => {
    ...
    const validationResult = await esgService.validateData(companyId);
  ...
  updateVerificationStatus = asyncHandler(async (req, res) => {
    ...
    const record = await esgService.updateVerificationStatus(
      companyId,
      status,
      userId,
      notes,
  ...
  getDataVersions = asyncHandler(async (req, res) => {
    ...
    const versions = await esgService.getDataVersions(companyId);
  ...
  restoreVersion = asyncHandler(async (req, res) => {
    ...
    const restoredRecord = await esgService.restoreVersion(
      companyId,
      versionId,
      userId,
    );
  ...
  createRecord = asyncHandler(async (req, res) => {
    ...
    const record = await esgService.createRecord(recordData, userId);

    res.status(201).json({
      success: true,
      message: "Overall ESG record created successfully",
      data: record,
    });
  });
  ...
  exportCSV = asyncHandler(async (req, res) => {
    ...
    const record = await esgService.getRecordById(null, companyId);
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=overall-esg-${companyId}-${
  ...
  bulkUpdateMetrics = asyncHandler(async (req, res) => {
    ...
    for (const metricData of metrics) {
      try {
        const record = await esgService.upsertMetric(
          companyId,
          metricData,
          userId,
        );
...
module.exports = new OverallESGController();
