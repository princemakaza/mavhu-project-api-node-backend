const governanceService = require("../services/governance_board_service");
...
function requireFields(body, fields = []) {
  ...
  if (missing.length) {
    throw new AppError("Missing required fields", 400, "MISSING_FIELDS", { missing });
  }
}

class GovernanceBoardController {
  /**
  ...
  importFile = asyncHandler(async (req, res) => {
    ...
    const record = await governanceService.importFromFile(
      req.file.buffer,
  ...
  importJSON = asyncHandler(async (req, res) => {
    ...
    if (!Array.isArray(req.body.data) && typeof req.body.data !== "object") {
      throw new AppError("Invalid data format. Must be array or object", 400, "INVALID_DATA_FORMAT");
    }
    ...
    const record = await governanceService.importFromJSON(
      req.body.data,
      companyId,
      userId,
      metadata
  ...
  /**
   * Get all governance & board records for a company
   */
  getCompanyRecords = asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const { include_inactive } = req.query;

    const records = await governanceService.getCompanyRecords(
      companyId,
  ...
  getRecordById = asyncHandler(async (req, res) => {
    ...
    const record = await governanceService.getRecordById(recordId, companyId);
  ...
  /**
   * Get metrics by category (e.g., board_composition, director_fees)
   */
  getMetricsByCategory = asyncHandler(async (req, res) => {
    const { companyId, category } = req.params;
    const { subcategory } = req.query;

    let metrics = await governanceService.getMetricsByCategory(companyId, category);

    if (!metrics || metrics.length === 0) {
      throw new AppError("No metrics found for category", 404, "NO_METRICS_FOUND");
    }
  ...
  /**
   * Get time series data for a metric (not heavily used here, but kept for consistency)
   */
  getTimeSeriesData = asyncHandler(async (req, res) => {
    const { companyId, metricName } = req.params;
    const { category } = req.query;

    const timeSeriesData = await governanceService.getTimeSeriesData(
      companyId,
      metricName,
      category
    );
    ...
    if (!timeSeriesData || timeSeriesData.length === 0) {
      throw new AppError("No time series data found", 404, "NO_TIME_SERIES_DATA");
    }
  ...
  upsertMetric = asyncHandler(async (req, res) => {
    ...
    const record = await governanceService.upsertMetric(companyId, metricData, userId);
  ...
  deleteMetric = asyncHandler(async (req, res) => {
    ...
    const record = await governanceService.deleteMetric(companyId, metricId, userId);
  ...
  getSummaryStats = asyncHandler(async (req, res) => {
    ...
    const summary = await governanceService.getSummaryStats(companyId);

    if (!summary) {
      throw new AppError("No governance & board data found for this company", 404, "NO_DATA_FOUND");
    }
  ...
  validateData = asyncHandler(async (req, res) => {
    ...
    const validationResult = await governanceService.validateData(companyId);
  ...
  updateVerificationStatus = asyncHandler(async (req, res) => {
    ...
    const record = await governanceService.updateVerificationStatus(
      companyId,
      status,
      userId,
      notes
  ...
  getDataVersions = asyncHandler(async (req, res) => {
    ...
    const versions = await governanceService.getDataVersions(companyId);
  ...
  restoreVersion = asyncHandler(async (req, res) => {
    ...
    const restoredRecord = await governanceService.restoreVersion(companyId, versionId, userId);

    if (!restoredRecord) {
      throw new AppError("Version not found or belongs to different company", 404, "VERSION_NOT_FOUND");
    }
  ...
  createRecord = asyncHandler(async (req, res) => {
    ...
    const record = await governanceService.createRecord(recordData, userId);

    res.status(201).json({
      success: true,
      message: "Governance & board record created successfully",
      data: record,
    });
  });
  ...
  exportCSV = asyncHandler(async (req, res) => {
    ...
    const record = await governanceService.getRecordById(null, companyId);
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=governance-board-${companyId}-${
   ...
   */
  convertToCSV(record) {
    const headers = ["Category", "Metric Name", "Item", "Details", "Source", "Verification Status"];
    const rows = [];

    record.metrics.forEach((metric) => {
      if (metric.data_type === "list" && metric.list_data) {
        metric.list_data.forEach((item) => {
          // For board composition, we have extra fields; we can flatten them
          if (metric.category === "board_composition") {
            rows.push([
              metric.category,
              metric.metric_name,
              item.name || "",
              `Role: ${item.role}, Type: ${item.type}, Tenure: ${item.tenure}, Appointed: ${item.appointed}, Skills: ${item.key_skills}`,
              item.source || "",
  ...
  bulkUpdateMetrics = asyncHandler(async (req, res) => {
    ...
    if (!Array.isArray(metrics) || metrics.length === 0) {
      throw new AppError("Metrics array is required and cannot be empty", 400, "INVALID_METRICS_ARRAY");
    }
...
module.exports = new GovernanceBoardController();