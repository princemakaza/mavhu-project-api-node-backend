const energyService = require("../services/energy_consumption_data_service");
...
class EnergyConsumptionController {
  ...
  importFile = asyncHandler(async (req, res) => {
    ...
    const record = await energyService.importFromFile(
      req.file.buffer,
  ...
  importJSON = asyncHandler(async (req, res) => {
    ...
    const record = await energyService.importFromJSON(
      req.body.data,
      companyId,
      userId,
      metadata,
  ...
  /**
   * Get all energy consumption records for a company
   */
  getCompanyRecords = asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const { include_inactive } = req.query;

    const records = await energyService.getCompanyRecords(
      companyId,
  ...
  getRecordById = asyncHandler(async (req, res) => {
    ...
    const record = await energyService.getRecordById(recordId, companyId);
  ...
  /**
   * Get metrics by category (e.g., bagasse_usage, fuel_consumption)
   */
  getMetricsByCategory = asyncHandler(async (req, res) => {
    const { companyId, category } = req.params;
    const { subcategory } = req.query;

    let metrics = await energyService.getMetricsByCategory(companyId, category);
  ...
  getTimeSeriesData = asyncHandler(async (req, res) => {
    ...
    const timeSeriesData = await energyService.getTimeSeriesData(
      companyId,
      metricName,
      category,
    );
  ...
  upsertMetric = asyncHandler(async (req, res) => {
    ...
    const record = await energyService.upsertMetric(
      companyId,
      metricData,
      userId,
    );
  ...
  deleteMetric = asyncHandler(async (req, res) => {
    ...
    const record = await energyService.deleteMetric(
      companyId,
      metricId,
      userId,
    );
  ...
  getSummaryStats = asyncHandler(async (req, res) => {
    ...
    const summary = await energyService.getSummaryStats(companyId);

    if (!summary) {
      throw new AppError(
        "No energy consumption data found for this company",
        404,
        "NO_DATA_FOUND",
      );
    }
  ...
  validateData = asyncHandler(async (req, res) => {
    ...
    const validationResult = await energyService.validateData(companyId);
  ...
  updateVerificationStatus = asyncHandler(async (req, res) => {
    ...
    const record = await energyService.updateVerificationStatus(
      companyId,
      status,
      userId,
      notes,
  ...
  getDataVersions = asyncHandler(async (req, res) => {
    ...
    const versions = await energyService.getDataVersions(companyId);
  ...
  restoreVersion = asyncHandler(async (req, res) => {
    ...
    const restoredRecord = await energyService.restoreVersion(
      companyId,
      versionId,
      userId,
    );
  ...
  createRecord = asyncHandler(async (req, res) => {
    ...
    const record = await energyService.createRecord(recordData, userId);
  ...
  exportCSV = asyncHandler(async (req, res) => {
    ...
    const record = await energyService.getRecordById(null, companyId);
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=energy-consumption-${companyId}-${
...
module.exports = new EnergyConsumptionController();
