const healthService = require("../services/health_safety_service");
...
class HealthSafetyController {
  ...
  importFile = asyncHandler(async (req, res) => {
    ...
    const record = await healthService.importFromFile(
      req.file.buffer,
  ...
  importJSON = asyncHandler(async (req, res) => {
    ...
    const record = await healthService.importFromJSON(
      req.body.data,
      companyId,
      userId,
      metadata,
  ...
  /**
   * Get all health & safety records for a company
   */
  getCompanyRecords = asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const { include_inactive } = req.query;

    const records = await healthService.getCompanyRecords(
      companyId,
  ...
  getRecordById = asyncHandler(async (req, res) => {
    ...
    const record = await healthService.getRecordById(recordId, companyId);
  ...
  /**
   * Get metrics by category (e.g., lti_metrics, health_services)
   */
  getMetricsByCategory = asyncHandler(async (req, res) => {
    const { companyId, category } = req.params;
    const { subcategory } = req.query;

    let metrics = await healthService.getMetricsByCategory(companyId, category);
  ...
  getTimeSeriesData = asyncHandler(async (req, res) => {
    ...
    const timeSeriesData = await healthService.getTimeSeriesData(
      companyId,
      metricName,
      category,
    );
  ...
  upsertMetric = asyncHandler(async (req, res) => {
    ...
    const record = await healthService.upsertMetric(
      companyId,
      metricData,
      userId,
    );
  ...
  deleteMetric = asyncHandler(async (req, res) => {
    ...
    const record = await healthService.deleteMetric(
      companyId,
      metricId,
      userId,
    );
  ...
  getSummaryStats = asyncHandler(async (req, res) => {
    ...
    const summary = await healthService.getSummaryStats(companyId);

    if (!summary) {
      throw new AppError(
        "No health & safety data found for this company",
        404,
        "NO_DATA_FOUND",
      );
    }
  ...
  validateData = asyncHandler(async (req, res) => {
    ...
    const validationResult = await healthService.validateData(companyId);
  ...
  updateVerificationStatus = asyncHandler(async (req, res) => {
    ...
    const record = await healthService.updateVerificationStatus(
      companyId,
      status,
      userId,
      notes,
  ...
  getDataVersions = asyncHandler(async (req, res) => {
    ...
    const versions = await healthService.getDataVersions(companyId);
  ...
  restoreVersion = asyncHandler(async (req, res) => {
    ...
    const restoredRecord = await healthService.restoreVersion(
      companyId,
      versionId,
      userId,
    );
  ...
  createRecord = asyncHandler(async (req, res) => {
    ...
    const record = await healthService.createRecord(recordData, userId);
  ...
  exportCSV = asyncHandler(async (req, res) => {
    ...
    const record = await healthService.getRecordById(null, companyId);
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=health-safety-${companyId}-${
...
module.exports = new HealthSafetyController();
