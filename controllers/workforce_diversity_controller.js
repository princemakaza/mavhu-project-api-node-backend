const workforceService = require("../services/workforce_diversity_service");
...
class WorkforceDiversityController {
  /**
  ...
  importFile = asyncHandler(async (req, res) => {
    ...
    const record = await workforceService.importFromFile(
      req.file.buffer,
  ...
  importJSON = asyncHandler(async (req, res) => {
    ...
    const record = await workforceService.importFromJSON(
      req.body.data,
      companyId,
      userId,
      metadata,
  ...
  /**
   * Get all workforce & diversity records for a company
   */
  getCompanyRecords = asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const { include_inactive } = req.query;

    const records = await workforceService.getCompanyRecords(
      companyId,
  ...
  getRecordById = asyncHandler(async (req, res) => {
    ...
    const record = await workforceService.getRecordById(recordId, companyId);
  ...
  /**
   * Get metrics by category (e.g., employee_data, recruitment_data)
   */
  getMetricsByCategory = asyncHandler(async (req, res) => {
    const { companyId, category } = req.params;
    const { subcategory } = req.query;

    let metrics = await workforceService.getMetricsByCategory(
      companyId,
      category,
    );
  ...
  getTimeSeriesData = asyncHandler(async (req, res) => {
    ...
    const timeSeriesData = await workforceService.getTimeSeriesData(
      companyId,
      metricName,
      category,
    );
  ...
  upsertMetric = asyncHandler(async (req, res) => {
    ...
    const record = await workforceService.upsertMetric(
      companyId,
      metricData,
      userId,
    );
  ...
  deleteMetric = asyncHandler(async (req, res) => {
    ...
    const record = await workforceService.deleteMetric(
      companyId,
      metricId,
      userId,
    );
  ...
  getSummaryStats = asyncHandler(async (req, res) => {
    ...
    const summary = await workforceService.getSummaryStats(companyId);

    if (!summary) {
      throw new AppError(
        "No workforce & diversity data found for this company",
        404,
        "NO_DATA_FOUND",
      );
    }
  ...
  validateData = asyncHandler(async (req, res) => {
    ...
    const validationResult = await workforceService.validateData(companyId);
  ...
  updateVerificationStatus = asyncHandler(async (req, res) => {
    ...
    const record = await workforceService.updateVerificationStatus(
      companyId,
      status,
      userId,
      notes,
  ...
  getDataVersions = asyncHandler(async (req, res) => {
    ...
    const versions = await workforceService.getDataVersions(companyId);
  ...
  restoreVersion = asyncHandler(async (req, res) => {
    ...
    const restoredRecord = await workforceService.restoreVersion(
      companyId,
      versionId,
      userId,
    );
  ...
  createRecord = asyncHandler(async (req, res) => {
    ...
    const record = await workforceService.createRecord(recordData, userId);

    res.status(201).json({
      success: true,
      message: "Workforce & diversity record created successfully",
      data: record,
    });
  });
  ...
  exportCSV = asyncHandler(async (req, res) => {
    ...
    const record = await workforceService.getRecordById(null, companyId);
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=workforce-diversity-${companyId}-${
...
module.exports = new WorkforceDiversityController();
