const fmcService = require("../services/fmc_service");
...
class FarmManagementComplianceController {
  /**
  ...
  importFile = asyncHandler(async (req, res) => {
    ...
    const record = await fmcService.importFromFile(
      req.file.buffer,
  ...
  importJSON = asyncHandler(async (req, res) => {
    ...
    const record = await fmcService.importFromJSON(
      req.body.data,
      companyId,
      userId,
      metadata,
  ...
  /**
   * Get all farm management compliance records for a company
   */
  getCompanyRecords = asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const { include_inactive } = req.query;

    const records = await fmcService.getCompanyRecords(
      companyId,
  ...
  getRecordById = asyncHandler(async (req, res) => {
    ...
    const record = await fmcService.getRecordById(recordId, companyId);
  ...
  getMetricsByCategory = asyncHandler(async (req, res) => {
    ...
    let metrics = await fmcService.getMetricsByCategory(companyId, category);
  ...
  getTimeSeriesData = asyncHandler(async (req, res) => {
    ...
    const timeSeriesData = await fmcService.getTimeSeriesData(
      companyId,
      metricName,
      category,
    );
  ...
  /**
   * Create or update a metric (upsert)
   */
  upsertMetric = asyncHandler(async (req, res) => {
    ...
    const record = await fmcService.upsertMetric(companyId, metricData, userId);
  ...
  deleteMetric = asyncHandler(async (req, res) => {
    ...
    const record = await fmcService.deleteMetric(companyId, metricId, userId);
  ...
  getSummaryStats = asyncHandler(async (req, res) => {
    ...
    const summary = await fmcService.getSummaryStats(companyId);

    if (!summary) {
      throw new AppError(
        "No farm management compliance data found for this company",
        404,
        "NO_DATA_FOUND",
      );
    }
  ...
  validateData = asyncHandler(async (req, res) => {
    ...
    const validationResult = await fmcService.validateData(companyId);
  ...
  updateVerificationStatus = asyncHandler(async (req, res) => {
    ...
    const record = await fmcService.updateVerificationStatus(
      companyId,
      status,
      userId,
      notes,
  ...
  getDataVersions = asyncHandler(async (req, res) => {
    ...
    const versions = await fmcService.getDataVersions(companyId);
  ...
  restoreVersion = asyncHandler(async (req, res) => {
    ...
    const restoredRecord = await fmcService.restoreVersion(
      companyId,
      versionId,
      userId,
    );
  ...
  /**
   * Create new farm management compliance record manually
   */
  createRecord = asyncHandler(async (req, res) => {
    ...
    const record = await fmcService.createRecord(recordData, userId);

    res.status(201).json({
      success: true,
      message: "Farm management compliance record created successfully",
      data: record,
    });
  });

  /**
   * Export farm management compliance data as CSV
   */
  exportCSV = asyncHandler(async (req, res) => {
    const { companyId } = req.params;

    const record = await fmcService.getRecordById(null, companyId);
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=farm-compliance-${companyId}-${
  ...
  bulkUpdateMetrics = asyncHandler(async (req, res) => {
    ...
    for (const metricData of metrics) {
      try {
        const record = await fmcService.upsertMetric(
          companyId,
          metricData,
          userId,
        );
...
module.exports = new FarmManagementComplianceController();
