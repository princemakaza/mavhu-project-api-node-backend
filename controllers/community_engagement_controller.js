const communityService = require("../services/community_engagement_data_service");
...
class CommunityEngagementController {
  ...
  importFile = asyncHandler(async (req, res) => {
    ...
    const record = await communityService.importFromFile(
      req.file.buffer,
  ...
  importJSON = asyncHandler(async (req, res) => {
    ...
    const record = await communityService.importFromJSON(
      req.body.data,
      companyId,
      userId,
      metadata,
  ...
  /**
   * Get all community engagement records for a company
   */
  getCompanyRecords = asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const { include_inactive } = req.query;

    const records = await communityService.getCompanyRecords(
      companyId,
  ...
  getRecordById = asyncHandler(async (req, res) => {
    ...
    const record = await communityService.getRecordById(recordId, companyId);
  ...
  /**
   * Get metrics by category (e.g., community_initiatives, social_welfare)
   */
  getMetricsByCategory = asyncHandler(async (req, res) => {
    const { companyId, category } = req.params;
    const { subcategory } = req.query;

    let metrics = await communityService.getMetricsByCategory(
      companyId,
      category,
    );
  ...
  /**
   * Get time series data for a metric (if applicable)
   */
  getTimeSeriesData = asyncHandler(async (req, res) => {
    const { companyId, metricName } = req.params;
    const { category } = req.query;

    const timeSeriesData = await communityService.getTimeSeriesData(
      companyId,
      metricName,
      category,
    );
  ...
  upsertMetric = asyncHandler(async (req, res) => {
    ...
    const record = await communityService.upsertMetric(
      companyId,
      metricData,
      userId,
    );
  ...
  deleteMetric = asyncHandler(async (req, res) => {
    ...
    const record = await communityService.deleteMetric(
      companyId,
      metricId,
      userId,
    );
  ...
  getSummaryStats = asyncHandler(async (req, res) => {
    ...
    const summary = await communityService.getSummaryStats(companyId);

    if (!summary) {
      throw new AppError(
        "No community engagement data found for this company",
        404,
        "NO_DATA_FOUND",
      );
    }
  ...
  validateData = asyncHandler(async (req, res) => {
    ...
    const validationResult = await communityService.validateData(companyId);
  ...
  updateVerificationStatus = asyncHandler(async (req, res) => {
    ...
    const record = await communityService.updateVerificationStatus(
      companyId,
      status,
      userId,
      notes,
  ...
  getDataVersions = asyncHandler(async (req, res) => {
    ...
    const versions = await communityService.getDataVersions(companyId);
  ...
  restoreVersion = asyncHandler(async (req, res) => {
    ...
    const restoredRecord = await communityService.restoreVersion(
      companyId,
      versionId,
      userId,
    );
  ...
  createRecord = asyncHandler(async (req, res) => {
    ...
    const record = await communityService.createRecord(recordData, userId);

    res.status(201).json({
      success: true,
      message: "Community engagement record created successfully",
      data: record,
    });
  });
  ...
  exportCSV = asyncHandler(async (req, res) => {
    ...
    const record = await communityService.getRecordById(null, companyId);
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=community-engagement-${companyId}-${
    ...
    record.metrics.forEach((metric) => {
      if (metric.data_type === "list" && metric.list_data) {
        metric.list_data.forEach((item) => {
          if (metric.category === "community_initiatives") {
            rows.push([
              metric.category,
              metric.metric_name,
              item.initiative || "",
...
module.exports = new CommunityEngagementController();
