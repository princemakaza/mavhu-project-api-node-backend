...
class WorkforceDiversityService {
  /**
  ...
  async createRecord(data, userId) {
    try {
      const record = new WorkforceDiversityData({
        ...data,
        created_by: userId,
  ...
  async importFromFile(fileBuffer, fileName, companyId, userId, metadata = {}) {
    try {
      ...
      const existingRecord = await WorkforceDiversityData.findOne({
        company: companyId,
        is_active: true,
      });
      ...
      if (existingRecord) {
        record = new WorkforceDiversityData({
          ...transformedData,
  ...
  async importFromJSON(jsonData, companyId, userId, metadata = {}) {
    try {
      ...
      if (existingRecord) {
        ...
        record = new WorkforceDiversityData(dataToImport);
      }
  ...
  transformParsedDataToSchema(parsedData, companyId, userId, metadata) {
    ...
    if (
      fileName.toLowerCase().includes("workforce") ||
      fileName.toLowerCase().includes("diversity") ||
      fileName.toLowerCase().includes("employee")
    ) {
      return this.processWorkforceCSVData(parsedData, companyId, userId, metadata);
    } else {
  ...
  /**
   * Process the specific workforce & diversity CSV format (your provided file)
   */
  processWorkforceCSVData(parsedData, companyId, userId, metadata) {
    const metrics = [];
    ...
    if (parsedData.raw_data && Array.isArray(parsedData.raw_data)) {
      ...
      let section = null; // null, "employee", "recruitment_gender", "recruitment_age", "turnover_age", "initiatives"
      ...
      for (const row of rows) {
        ...
        if (firstColStr === "Employee Data by Gender") {
          section = "employee";
          continue;
        }
        if (firstColStr === "Recruitment Data by Gender") {
          section = "recruitment_gender";
          continue;
        }
        if (firstColStr === "Recruitment by Age Group") {
          section = "recruitment_age";
          continue;
        }
        if (firstColStr === "Employee Turnover by Age Group") {
          section = "turnover_age";
          continue;
        }
        if (firstColStr === "Diversity & Inclusion Initiatives:") {
          section = "initiatives";
          continue;
        }

        // Skip rows that are just empty or separators
        ...
        if (section === "employee") {
          ...
          const femaleKey = "Female Employees";
          const maleKey = "Male Employees";

          const femaleVal = row[femaleKey];
          if (femaleVal !== undefined && femaleVal !== "") {
            const numericValue = parseNumber(femaleVal);
            let metric = metrics.find(m => m.category === "employee_data" && m.subcategory === "female_employees");
            if (!metric) {
              metric = {
                category: "employee_data",
                subcategory: "female_employees",
                metric_name: "Female Employees",
          ...
          const maleVal = row[maleKey];
          if (maleVal !== undefined && maleVal !== "") {
            const numericValue = parseNumber(maleVal);
            let metric = metrics.find(m => m.category === "employee_data" && m.subcategory === "male_employees");
            if (!metric) {
              metric = {
                category: "employee_data",
                subcategory: "male_employees",
                metric_name: "Male Employees",
                data_type: "yearly_series",
                yearly_data: [],
                created_by: userId
              };
        ...
        } else if (section === "recruitment_gender") {
          // Recruitment Data by Gender
          const year = row["Year"] || row[Object.keys(row)[0]];
          if (!year || year === "Year") continue;

          const yearStr = year.toString().trim();
          const femaleRecruitKey = "Female Recruits";
          const maleRecruitKey = "Male Recruits";

          const femaleVal = row[femaleRecruitKey];
          if (femaleVal !== undefined && femaleVal !== "") {
            const numericValue = parseNumber(femaleVal);
            let metric = metrics.find(m => m.category === "recruitment_data" && m.subcategory === "female_recruits");
            if (!metric) {
              metric = {
                category: "recruitment_data",
                subcategory: "female_recruits",
                metric_name: "Female Recruits",
                data_type: "yearly_series",
                yearly_data: [],
                created_by: userId
              };
          ...
          const maleVal = row[maleRecruitKey];
          if (maleVal !== undefined && maleVal !== "") {
            const numericValue = parseNumber(maleVal);
            let metric = metrics.find(m => m.category === "recruitment_data" && m.subcategory === "male_recruits");
            if (!metric) {
              metric = {
                category: "recruitment_data",
                subcategory: "male_recruits",
                metric_name: "Male Recruits",
                data_type: "yearly_series",
                yearly_data: [],
                created_by: userId
              };
        ...
        } else if (section === "recruitment_age") {
          ...
          const yearStr = year.toString().trim();
          const ageGroups = [
            { key: "Under 30 years", sub: "under_30", metric: "Under 30 years" },
            { key: "30 to 50 years", sub: "30_to_50", metric: "30 to 50 years" },
            { key: "Over 50 years", sub: "over_50", metric: "Over 50 years" }
          ];

          for (const ag of ageGroups) {
            const val = row[ag.key];
            if (val !== undefined && val !== "") {
              const numericValue = parseNumber(val);
              let metric = metrics.find(m => m.category === "recruitment_by_age" && m.subcategory === ag.sub);
              if (!metric) {
                metric = {
                  category: "recruitment_by_age",
                  subcategory: ag.sub,
        ...
        } else if (section === "turnover_age") {
          // Employee Turnover by Age Group
          const year = row["Year"] || row[Object.keys(row)[0]];
          if (!year || year === "Year") continue;

          const yearStr = year.toString().trim();
          const turnoverGroups = [
            { key: "18 to 35 years", sub: "18_to_35", metric: "18 to 35 years" },
            { key: "36 to 50 years", sub: "36_to_50", metric: "36 to 50 years" },
            { key: "51 to 54 years", sub: "51_to_54", metric: "51 to 54 years" },
            { key: "55 to 65 years", sub: "55_to_65", metric: "55 to 65 years" }
          ];

          for (const tg of turnoverGroups) {
            const val = row[tg.key];
            if (val !== undefined && val !== "") {
              const numericValue = parseNumber(val);
              let metric = metrics.find(m => m.category === "turnover_by_age" && m.subcategory === tg.sub);
              if (!metric) {
                metric = {
                  category: "turnover_by_age",
                  subcategory: tg.sub,
                  metric_name: tg.metric,
                  data_type: "yearly_series",
                  yearly_data: [],
                  created_by: userId
                };
          ...
          // Diversity & Inclusion Initiatives: bullet points
          if (firstColStr.startsWith("•") || firstColStr.startsWith("-")) {
            let initiativesMetric = metrics.find(m => m.category === "diversity_initiatives" && m.metric_name === "Diversity & Inclusion Initiatives");
            if (!initiativesMetric) {
              initiativesMetric = {
                category: "diversity_initiatives",
                metric_name: "Diversity & Inclusion Initiatives",
                data_type: "list",
                list_data: [],
                created_by: userId
              };
            ...
            // Also, some lines may contain numeric data like "33% female representation" – we could extract that as a single value, but for simplicity store as list item.
            ...
            // Special handling for the training line: "Training: Female avg 7 hours, Male avg 10 hours (2025)"
            if (firstColStr.includes("Training:")) {
              // Optionally create separate metrics for average training hours
              // For now, just keep as list item.
            }
          }
        }
      }

      // Additionally, we might want to create a separate metric for the 33% female representation in external appointments
      // But that is included in the list. Could also create a single_value metric.
    }
  ...
  async getCompanyRecords(companyId, includeInactive = false) {
    try {
      ...
      return await WorkforceDiversityData.find(query)
  ...
  async getRecordById(recordId, companyId) {
    try {
      ...
      return await WorkforceDiversityData.findOne(query)
        .populate("created_by", "name email")
        .populate("last_updated_by", "name email");
    } catch (error) {
  ...
  async getMetricsByCategory(companyId, category) {
    try {
      const record = await WorkforceDiversityData.findOne({
        company: companyId,
        is_active: true,
      });
      if (!record) return [];
  ...
  async upsertMetric(companyId, metricData, userId) {
    try {
      ...
      try {
        let record = await WorkforceDiversityData.findOne({
          company: companyId,
          is_active: true,
        }).session(session);
  ...
  async getDataVersions(companyId) {
    try {
      return await WorkforceDiversityData.find({ company: companyId })
  ...
  async restoreVersion(companyId, versionId, userId) {
    try {
      ...
      try {
        const versionToRestore = await WorkforceDiversityData.findById(versionId).session(session);
        ...
        const currentActive = await WorkforceDiversityData.findOne({
          company: companyId,
          is_active: true,
        }).session(session);
        ...
        const restoredRecord = new WorkforceDiversityData({
...
module.exports = new WorkforceDiversityService();