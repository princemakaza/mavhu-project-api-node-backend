 ...
 * from the EnergyConsumptionData model.
 */
async function getAllMetricsByYear(companyId, years = []) {
  try {
    ...
    if (years.length > 0) {
      query["metrics.yearly_data.year"] = { $in: years };
    }

    const energyDocs = await EnergyConsumptionData.find(query)
    ...
    energyDocs.forEach((doc) => {
      doc.metrics.forEach((metric) => {
        ...
        (metric.yearly_data || []).forEach((yd) => {
          if (years.length === 0 || years.includes(yd.year)) {
        ...
        // Handle single_value metrics
        if (metric.data_type === "single_value" && metric.single_value) {
          if (!allMetrics[metricName]) {
            allMetrics[metricName] = {
              name: metricName,
              category: metric.category,
              unit: metric.single_value.unit || "",
              description: metric.description || "",
              values: [],
            };
          }
          ...
          allMetrics[metricName].values.push({
            ...
            value: metric.single_value.value,
            numeric_value: metric.single_value.numeric_value,
            source_notes:
              metric.single_value.source || metric.single_value.notes,
            added_at: metric.single_value.added_at,
            last_updated_at: metric.single_value.added_at,
          });
        }

        // Handle list_data
        if (metric.data_type === "list" && metric.list_data) {
          metric.list_data.forEach((item) => {
            const listMetricName = `${metricName} - ${item.item}`;
            if (!allMetrics[listMetricName]) {
              allMetrics[listMetricName] = {
                name: listMetricName,
                category: metric.category,
                unit: "",
            ...
            allMetrics[listMetricName].values.push({
              year: new Date().getFullYear().toString(),
              value: item.count,
              numeric_value: item.count,
              source_notes: item.source,
              added_at: item.added_at,
              last_updated_at: item.added_at,
            });
          });
        }
      });
...
function calculateTrend(metric, years) {
  if (!metric || !metric.values || metric.values.length < 2) return "stable";
...
async function getEnergyRenewablesData(companyId, year) {
  ...
  // Fetch the active EnergyConsumptionData record(s) for the company
  const energyDataRecord = await EnergyConsumptionData.findOne({
    company: companyId,
    is_active: true,
  }).lean();
  ...
  const allMetricsCurrentYear = await getAllMetricsByYear(companyId, [
    currentYear,
  ]);
  ...
  const totalEnergyGJ =
    totalEnergy > 0 ? totalEnergy : renewableEnergyGJ + fossilEnergyGJ;

  // Calculate percentages
  const renewablePercentage =
    totalEnergyGJ > 0 ? (renewableEnergyGJ / totalEnergyGJ) * 100 : 0;
  const fossilPercentage =
    totalEnergyGJ > 0 ? (fossilEnergyGJ / totalEnergyGJ) * 100 : 0;

  // Calculate renewable energy mix percentages
  const bagassePercentage =
    renewableEnergyGJ > 0 ? ((bagasseUsage * 10) / renewableEnergyGJ) * 100 : 0;
  const solarPercentage =
    renewableEnergyGJ > 0
      ? ((solarUsage * 0.0036) / renewableEnergyGJ) * 100
      : 0;

  // Calculate fossil energy mix percentages
  const coalPercentage =
    fossilEnergyGJ > 0 ? ((coalConsumption * 25) / fossilEnergyGJ) * 100 : 0;
  const dieselPercentage =
    fossilEnergyGJ > 0 ? ((dieselUsage * 0.036) / fossilEnergyGJ) * 100 : 0;

  // Calculate trends
  const coalTrend = calculateTrend(
    energyMetrics["Energy Consumption - Coal Consumption (tons)"],
    trendYears,
  );
  const dieselTrend = calculateTrend(
    energyMetrics["Energy Consumption - Inside Company Diesel Usage (litres)"],
    trendYears,
  );
  const renewableTrend = calculateTrend(
    energyMetrics["Renewable Energy Percentage (%)"],
    trendYears,
  );

  // Calculate grid dependency
  const totalElectricityConsumption =
    electricityGenerated + electricityPurchased - electricityExported;
  const gridDependencyPercentage =
    totalElectricityConsumption > 0
      ? ((electricityPurchased - electricityExported) /
  ...
  const energyCost =
    getMetricValueByYear(energyMetrics["Energy Cost (USD)"], currentYear) || 0;
  const costSavings = renewableEnergyGJ > 0 ? renewableEnergyGJ * 10 : 0;
  const carbonCostAvoided = renewableEnergyGJ * 0.05 * 50;
  ...
  const completenessScore =
    totalExpectedMetrics > 0
      ? Math.round((availableMetrics / totalExpectedMetrics) * 100)
      : 0;


  // Default carbon emissions (since CarbonEmissionAccounting model is removed)
  const defaultCarbonEmissions = {
    year: currentYear,
    emissions: {
      scope2: {
        total_tco2e_per_ha: 0,
        total_tco2e: 0,
        sources: [],
      },
      scope3: {
  ...
  const data = {
    company: company,
    // Include the raw EnergyConsumptionData record(s)
    energy_consumption_data: energyDataRecord || null,
    ...
    kpis: {
      ...
      energy_intensity:
        getMetricValueByYear(
          energyMetrics["Energy Intensity (GJ/unit production)"],
          currentYear,
        ) || 0,
      total_energy_consumption_gj: totalEnergyGJ.toFixed(0),
      renewable_energy_generation_gj: renewableEnergyGJ.toFixed(0),
      carbon_intensity_tco2e_per_gj:
        totalEnergyGJ > 0
          ? (
              (defaultCarbonEmissions.emissions.scope2.total_tco2e || 0) /
              totalEnergyGJ
            ).toFixed(3)
          : 0,
      clean_energy_transition_score:
    ...
    energy_mix: {
      renewable_sources: {
        ...
        breakdown: {
          ...
          other_renewables: Math.max(
            0,
            100 - bagassePercentage - solarPercentage,
          ).toFixed(1),
        },
      ...
      fossil_sources: {
        ...
        breakdown: {
          ...
          other_fossil: Math.max(
            0,
            100 - coalPercentage - dieselPercentage,
          ).toFixed(1),
        },
    ...
    grid_operations: {
      ...
      net_grid_import_mwh: (electricityPurchased - electricityExported).toFixed(
        0,
      ),
      grid_self_sufficiency_percentage:
        electricityGenerated + electricityPurchased > 0
          ? (
              (electricityGenerated /
                (electricityGenerated + electricityPurchased)) *
              100
            ).toFixed(1)
          : 0,
    ...
    // Carbon Emissions from Energy (now default)
    carbon_emissions: defaultCarbonEmissions,
    ...
    financial_impact: {
      ...
      roi_percentage:
        energyCost > 0
          ? (((costSavings + carbonCostAvoided) / energyCost) * 100).toFixed(1)
          : 0,
      energy_cost_per_gj:
        totalEnergyGJ > 0 ? (energyCost / totalEnergyGJ).toFixed(2) : 0,
    },
        ...
        datasets: [
          {
            data: [
              bagassePercentage,
              solarPercentage,
              Math.max(0, 100 - bagassePercentage - solarPercentage),
            ],
    ...
    data_quality: {
      ...
      notes:
        completenessScore < 100
          ? `Missing ${energyMetricNames.length - Object.keys(energyMetrics).length} energy metrics`
          : "All energy metrics available",
