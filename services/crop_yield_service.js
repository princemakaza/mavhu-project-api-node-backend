const CropYieldData = require("../models/crop_yield_model");
...
class CropYieldService {
  ...
  async createRecord(data, userId) {
    try {
      const record = new CropYieldData({
        ...data,
        created_by: userId,
  ...
  async importFromFile(fileBuffer, fileName, companyId, userId, metadata = {}) {
    try {
      ...
      const existingRecord = await CropYieldData.findOne({
        company: companyId,
        is_active: true,
      });
  ...
  async importFromJSON(jsonData, companyId, userId, metadata = {}) {
    try {
      ...
      if (existingRecord) {
        ...
        record = new CropYieldData(dataToImport);
      }
  ...
  transformParsedDataToSchema(parsedData, companyId, userId, metadata) {
    ...
    if (
      ...
      fileName.toLowerCase().includes("yield")
    ) {
      return this.processCropYieldCSVData(
        parsedData,
        companyId,
        userId,
        metadata,
  ...
  /**
   * Process crop yield specific CSV format (Hippo Valley Estates style)
   */
  processCropYieldCSVData(parsedData, companyId, userId, metadata) {
    const metrics = [];
    ...
    if (parsedData.raw_data && Array.isArray(parsedData.raw_data)) {
      ...
      let inYoYSection = false;
      const categoryMapping = {
        "Cane Harvested": "cane_harvested",
        "Sugar Production": "sugar_production",
        "Sugar Cane Yield": "sugar_cane_yield",
        "Area Under Cane": "area_under_cane",
      };

      // Helper to extract unit from metric name
      const extractUnit = (metricName) => {
      ...
      // Process main data rows (before YoY section)
      for (const row of rows) {
        ...
        if (typeof firstCol === "string") {
          if (firstCol.includes("Year-over-Year Changes")) {
            inYoYSection = true;
            continue;
          }
        }
        ...
        if (!inYoYSection) {
          // Expect row format: Category, Metric, 2022, 2023, 2024, 2025
          const categoryRaw = row["Category"] || row[Object.keys(row)[0]];
          const metricName = row["Metric"] || row[Object.keys(row)[1]];
          if (!categoryRaw || !metricName) continue;

          const category = categoryMapping[categoryRaw.trim()];
          if (!category) continue; // skip unrecognized

          // Determine subcategory
          let subcategory = "";
          if (metricName.includes("Company's Own Estates"))
            subcategory = "company_estates";
          else if (metricName.includes("Private Farmers"))
            subcategory = "private_farmers";
          else if (metricName.includes("Total")) subcategory = "total";
          else if (metricName.includes("Molasses")) subcategory = "molasses";
          else if (metricName.includes("Cane to Sugar Ratio"))
            subcategory = "cane_to_sugar_ratio";

          const unit = extractUnit(metricName);
          ...
          if (metric) {
            metric.yearly_data.push(...yearlyData);
          } else {
            metrics.push({
              category,
              subcategory,
          ...
          // Process Year-over-Year changes
          // Row format: Metric, 2022→2023, 2023→2024, 2024→2025
          ...
          const periods = {
            "2022→2023":
              row["2022→2023"] || row["2022→2023"] || row[Object.keys(row)[1]],
            "2023→2024": row["2023→2024"] || row[Object.keys(row)[2]],
            "2024→2025": row["2024→2025"] || row[Object.keys(row)[3]],
          };
  ...
  async getCompanyRecords(companyId, includeInactive = false) {
    try {
      ...
      return await CropYieldData.find(query)
  ...
  async getRecordById(recordId, companyId) {
    try {
      ...
      return await CropYieldData.findOne(query)
        .populate("created_by", "name email")
        .populate("last_updated_by", "name email");
    } catch (error) {
  ...
  async getTimeSeriesData(companyId, metricName, category = null) {
    try {
      ...
      const query = { metric_name: metricName, is_active: true };
  ...
  async upsertMetric(companyId, metricData, userId) {
    try {
      ...
      try {
        let record = await CropYieldData.findOne({
          company: companyId,
          is_active: true,
        }).session(session);
  ...
  async getDataVersions(companyId) {
    try {
      return await CropYieldData.find({ company: companyId })
        .select(
    ...
    try {
      ...
      try {
        const versionToRestore =
          await CropYieldData.findById(versionId).session(session);
        if (
          !versionToRestore ||
        ...
        const currentActive = await CropYieldData.findOne({
          company: companyId,
          is_active: true,
        }).session(session);
        ...
        const restoredRecord = new CropYieldData({
...
module.exports = new CropYieldService();
